...
float fHz_Max = 2000;        // 周波数[Hz]
float fHz_Min = 0.1;       // 周波数[Hz]
...
void setup() {
  ...
  iOnDelay_ms = lOntime / 1000;
  iOnDelay_us = lOntime % 1000;

  iOffDelay_ms = lOfftime / 1000;
  iOffDelay_us = lOfftime % 1000;
...
// delay(5000);
  ...
  if (iMode == 1){
    ...
    if (iOnOff == 0){
      lcd.print("OffFr:");     // LCD書込み
    }else{
      lcd.print("On Fr:");     // LCD書込み
    } 
    ...
    if (fDuty < 1){
      ...
      lcd.setCursor(7, 1);    // LCD書込み開始位置ｾｯﾄ
    }
